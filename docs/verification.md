...
6. **Gate-level.** Tiny Tapeout `gds.yaml` GL job simulates the LibreLane netlist. Locally, `scripts/synth.sh` also writes `build/synth.v` for a flattened behavioural netlist.
